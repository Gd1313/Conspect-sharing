...
namespace Conspect_sharing.Services
{
    ...
    public IEnumerable<ArticleModel> GetIndexedArticles(string keyword)
    {
        ...
        if (keyword != null)
        {
            result.AddRange(_searchRepository.ExecuteSqlQuery("Articles", "Text", keyword));
            ...
            result.AddRange(_searchRepository.ExecuteSqlQuery("Articles", "Specialty", keyword));
    ...
    {
        TagModel tag = _tagRepository.Get(new Guid(hashtag));
        if (tag != null)
        {
